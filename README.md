### Hi there 👋

HENNADII HALANETS – SYSTEMS ENGINEER


SUMMARY

• Systems Engineer at EPAM UA with professional experience in designing and building infrastructure for continuous integration and delivery in the cloud.  
• Proficient in containerization, Infrastructure as Code (IaC), Configuration Management, Cloud solutions, CI/CD pipelines, databases, monitoring, and alerting tools.  
• Skilled in building custom images, setting up CI/CD pipelines, documenting processes, and effectively sharing project knowledge.  
• Acting Head of IT Helpdesk Unit, successfully managing a team of 5 members and providing IT support to over 500 employees.  
• Held diverse roles, including 2nd Line/Environment Support Engineer, Application Support Engineer, Cloud Application Support, System Administrator, and Helpdesk Engineer, demonstrating adaptability and a broad skill set.  


SKILLS

Technologies

•	CI/CD & SCM: Jenkins, GitLab CI, GitHub, GitLab
•	IaC: Ansible, Terraform
•	Cloud: Amazon Web Services (EC2, VPC, S3, IAM, Route 53, ELB, RabbitMQ, Elastic Container Registry, RDS, Simple Notification Service, AWS Simple Queue Service, Simple Email Service)
•	Containers: Docker, Kubernetes, Helm, Rancher, Amazon Elastic Kubernetes Service
•	Scripting: bash, groovy, Python
•	Web/Application Server: nginx, Apache HTTP Server
•	Database: PostgreSQL, Amazon RDS
•	Monitoring tools: ELK, EFK, Prometheus, Grafana, VictoriaMetrics
•	OS: Windows, Linux, macOS
•	Keys and Secrets Management: HashiCorp Vault
•	Teamwork: MS Teams, Jira, Confluence, Skype, Discord, etc

English: B1

Academic Disciplines

•	Applied Sciences: Systems engineering, Application and Web Servers, Engineering, General programming concepts

Leadership & Soft Skills

•	Communication: Meeting Facilitation, Communication, Conflict management, Building dialogue
•	Teamwork and Collaboration: Team Management
•	Growth Mindset: Adaptability
•	Ownership: Problem-solving, Time Management, Ownership
•	Consultancy: Consultancy, Client Relationship Management, Delivery Excellence
•	Business Acumen: Market orientation, Financial Awareness
•	General Management: Configuration Management

Managerial

•	General Management: Configuration Management
•	People Management: People Management


WORK EXPERIENCE

Jun-2021 - Till Now - Systems Engineer, EPAM Systems, https://www.epam.com/

Sep-2019 - Jun-2021 - Acting Head of IT Helpdesk Unit, Viseven, https://viseven.com/

Jun-2018 - Sep-2019 - System administrator; Helpdesk engineer, Viseven, https://viseven.com/

Aug-2017 - Jan-2018 - Computer Systems Engineer; Electronics engineer, Builder’s Training College ZNAEU, http://bkzt.at.ua/

Nov-2016 - Jan-2018 - Mentor, BrainBasket Foundation, https://brainbasket.org/

Apr-2016 - Jul-2017 - Specialist of the department of management of requests and monitoring of IT of the department of technical support of the Department of information technologies, Public joint-stock company "Sberbank", https://www.sberbank.ua/

Mar-2001 - Apr-2016 - Teacher; Network Administrator; System Administrator, Zhytomyr Technological College, http://www.ztk.org.ua/


EDUCATION
Name of the Education Establishment: ZHYTOMYR POLYTECHNIC STATE UNIVERSITY (1997-2001)
Faculty/College: Information and Computer Technology
Specialty: Automation

Name of the Education Establishment: ZHYTOMYR COLLEGE OF TECHNOLOGY (1993-1997)
Faculty/College: Automation
Specialty: Automation
